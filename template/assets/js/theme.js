    ...
    $.fn.customFile = function () {
        return this.each(function () {
            ...
            if (
                ...
                var _txtB =
                    '<i class="ic ic-upload c-theme size-lg"></i> <span>';
            }
...
jQuery(function ($) {
    ...
    function showTab(elem) {
        ...
        if ($(elem).parents(".tab-container").hasClass("offset-sction")) {
            $("html,body").animate(
                {
                    scrollTop:
                        $(elem).closest("section").offset().top - _windowHeight,
                },
                800
            );
        } else {
    ...
    $(".tab-container select.tab-select2").change(function () {
        ...
        $('.tab-container .tab a[href="' + $(this).val() + '"]').addClass(
            "active"
        );
        showTab($(this).val());
    });
...
jQuery(function ($) {
    if ($(".sc-menu.swiper").length) {
        $(".sc-menu.swiper").each(function () {
            ...
            if (
                _this.find(".swiper-slide.current-cat").length &&
                slideTotal > 2
            ) {
                _this.each(function () {
                    var _current =
                        $(this).find(".swiper-slide.current-cat").index() - 1;
                    swiper.slideTo(_current, 0, false);
                });
            }

            if (
                _this.find(".swiper-slide.is-active").length &&
                slideTotal > 2
            ) {
                _this.each(function () {
                    var _current =
                        $(this).find(".swiper-slide.is-active").index() - 1;
                    swiper.slideTo(_current, 0, false);
                });
            }

            if (
                _this.find(".swiper-slide > .active").length &&
                slideTotal > 2
            ) {
                _this.each(function () {
                    var _current =
...
jQuery(document).ready(function () {
    $(".screen").each(function () {
        const popup = $(this);
        const overlayClose = popup.find(".overlay-close");
